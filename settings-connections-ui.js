(()=>{
  ...
  function render(){
    ...
    // promptai:access feuert bei jeder Hintergrund-Synchronisation, nicht nur bei echten
    // Aenderungen. Das Markup unten oeffnet aber nur den aktiven Platz - ein Platz, den jemand
    // gerade von Hand aufgeklappt hat, fiel deshalb beim naechsten Abgleich wieder zu, mitten
    // im Eintragen eines Schluessels. Was offen ist, bleibt offen.
    const openIndexes=new Set($$('.conn-slot[open]',box).map(node=>node.dataset.connSlot));
    const used=new Set();
