(()=>{
  ...
  function render(){
    ...
    // promptai:access fires on every background sync, not just on real changes - without this,
    // a slot someone just opened by hand fell shut again on the very next poll, because the
    // rebuilt markup below only ever opens the active slot.
    const openIndexes=new Set($$('.conn-slot[open]',box).map(d=>d.dataset.connSlot));
