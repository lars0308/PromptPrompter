(()=>{
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='streamlinedFlowStyles';s.textContent=`
      ...
      html:not([data-prompt-mode="expert"]) #stepPreviews .preview-step-head{gap:16px}html:not([data-prompt-mode="expert"]) #stepPreviews .preview-generation-controls{padding:12px;border:1px solid var(--line);border-radius:14px;background:var(--surface)}html[data-prompt-mode="guided"] #stepPreviews .preview-generation-controls{grid-template-columns:100px minmax(0,1fr)!important}
      @media(max-width:760px){.streamline-working{min-height:260px;padding:30px 14px}html[data-prompt-mode="guided"] #stepPreviews .preview-generation-controls{grid-template-columns:94px minmax(0,1fr)!important}html[data-prompt-mode="guided"] #stepPreviews #generateConceptsBtn{grid-column:1/-1;width:100%!important}}
