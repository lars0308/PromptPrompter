(()=>{
  ...
  function ensureTransition(){const workspace=$('#workflowApp .workspace');if(!workspace)return null;let box=$('#flowTransitionCompact');if(box)return box;box=document.createElement('section');box.id='flowTransitionCompact';box.className='flow-transition-compact';box.innerHTML='<div><span>RÜCKMELDUNG</span><strong>Briefing wird kurz geprüft</strong><small>Nur offene Punkte, die die Vorschau wirklich verändern, werden gleich abgefragt.</small><i></i></div>';workspace.prepend(box);return box}
  function syncFlow(){
    const n=currentStep(),active=simpleMode()&&workflowVisible();document.documentElement.classList.toggle('prompt-simple-workflow',active);const transition=active&&[3,4,5].includes(n)&&!$('#clarificationDialog')?.open;document.documentElement.classList.toggle('prompt-review-transition',transition);const box=ensureTransition();
    if(box){box.classList.toggle('show',transition);const label=$('span',box),strong=$('strong',box),small=$('small',box);if(n===3){setText(label,'RÜCKMELDUNG');setText(strong,'Briefing wird kurz geprüft');setText(small,'Nur offene Punkte, die die Vorschau wirklich verändern, werden abgefragt.')}else{setText(label,'VORSCHAU');setText(strong,'Vorschau wird vorbereitet');setText(small,'Deine Antworten und Referenzen werden jetzt zusammengeführt.')}}
    if(active&&n===2){const next=$('#stepReferences .next-btn'),copy='Rückmeldung prüfen <i>→</i>';if(next&&next.innerHTML!==copy)next.innerHTML=copy}
