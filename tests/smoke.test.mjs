...
test('mobile menu is structured and dark mode only gets a quick top button',async()=>{const src=await text('ux-stability-fix.js');for(const token of ['menuLibrariesBtn','Projekte','Profil','Einstellungen','Abmelden','menuThemeQuick'])assert.ok(src.includes(token),token);assert.match(src,/#themeToggleBtn\{display:none!important\}/);assert.match(src,/topbar-menu #resetBtn\{display:none!important\}/);assert.doesNotMatch(src,/setText\(upgrade,'Upgraden'\)/)});
...
test('free prompt has second settings stage with editable description and no leftover legacy generation stage',async()=>{const src=await text('ui-polish-final.js');for(const token of ['SCHRITT 2 · EINSTELLUNGEN','free-prompt-brief-card','free-description-collapsed','dataset.editing'])assert.ok(src.includes(token),token);assert.doesNotMatch(src,/tier-locked/);assert.doesNotMatch(src,/free-prompt-generation-stage/)});
