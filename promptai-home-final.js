(()=>{
  ...
  // Zwei Wege statt einem: Android und iOS zeigen bei einer gemischten Dateiliste nur die
  // Auswahl "Foto aufnehmen / Dateien" - die Galerie fehlt dort. Steht im accept nur image/*,
  // oeffnet dasselbe Feld direkt die Bildauswahl des Geraets.
  function attachFile(nurBilder=false){
    ...
    input.accept=nurBilder?'image/*':(target.getAttribute('accept')||'');
  ...
  // Die Mindestlaenge stand frueher als minlength am Textfeld. Damit uebernahm der Browser die
  // Pruefung: bei ein bis sieben Zeichen blockierte er das Absenden und zeigte seine eigene
  // Sprechblase - unsere Zeile darunter kam nie dazu. Bei leerem Feld greift minlength nicht,
  // dort erschien sie doch. Zwei verschiedene Meldungen fuer denselben Fall. Jetzt prueft nur
  // noch submitCommand, und die Meldung steht immer an derselben Stelle.
  ...
  function bindHome(home){home.dataset.commandMode='website';$('#promptModeButton',home).addEventListener('click',()=>{const menu=$('#promptModeMenu',home),open=menu.hidden;menu.hidden=!open;$('#promptModeButton',home).setAttribute('aria-expanded',String(open))});home.querySelectorAll('[data-command-mode]').forEach(option=>option.addEventListener('click',()=>selectMode(option.dataset.commandMode)));$('#promptCommandForm',home).addEventListener('submit',submitCommand);
    ...
    home.querySelectorAll('[data-attach]').forEach(button=>button.addEventListener('click',()=>{
      ...
      const art=button.dataset.attach;
      if(art==='file')attachFile();else if(art==='image')attachFile(true);else attachUrl();
    }));
    document.addEventListener('click',event=>{
      // Nur echte Klicks schliessen. Die App klickt selbst - restore() in stability-ui.js stellt
      // nach dem Aufloesen der Sitzung den gespeicherten Ablauf ueber einen Klick her. Dieser
