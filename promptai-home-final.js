(()=>{
  ...
  function installStyles(){
    ...
    const style=document.createElement('style');style.id='promptAiHomeFinalStyles';style.textContent=`
      ...
      .prompt-command-top{position:relative;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:18px 18px 0}
      .prompt-mode-button{display:inline-flex;align-items:center;gap:10px;height:44px;min-height:44px;padding:0 13px;border:1px solid #425262;border-radius:12px;background:#151f29;color:#edf6fd;font:750 13px/1 Arial,Helvetica,sans-serif;cursor:pointer}
      ...
      .prompt-command-submit{position:absolute;right:23px;bottom:73px;display:grid;width:44px;height:44px;min-height:44px;padding:0;place-items:center;border:0;border-radius:12px;background:var(--home-blue);color:#07131c;cursor:pointer;transition:transform .16s ease,background .16s ease}
      ...
      .prompt-command-submit svg{width:18px;height:18px}
      ...
      .prompt-attach-button{display:grid;place-items:center;width:44px;height:44px;padding:0;border:1px solid #33465a;border-radius:12px;background:transparent;color:#c8d6e2;font:400 22px/1 Arial,Helvetica,sans-serif;cursor:pointer}
      .prompt-attach-button:hover{border-color:#4d637a;background:rgba(255,255,255,.06);color:#e7f1f9}
      ...
      /* Stand früher als Textzeile im Fußbereich ("Mit Rückfragen · Vorlagenname") - auf dem
         Handy brach das mitten im Wort um und stand neben Kontingent und Tarif zu eng. Jetzt nur
         noch ein Symbol oben neben der Arbeitsart; die Kurzbeschreibung bleibt für Screenreader
         erhalten, ist aber nicht mehr sichtbarer Text. */
      .prompt-setup-line{position:relative;flex:0 0 auto;display:grid;place-items:center;width:44px;height:44px;padding:0;border:1px solid #33465a;border-radius:12px;background:transparent;color:#c8d6e2;cursor:pointer}
      .prompt-setup-line:hover,.prompt-setup-line[aria-expanded="true"]{border-color:#4d637a;color:#e7f1f9;background:rgba(255,255,255,.04)}
      .prompt-setup-line>span{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
      .prompt-setup-line .mode-chevron{display:none}
      .prompt-setup-gear{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.6;stroke-linecap:round;stroke-linejoin:round}
      ...
      /* Ein echtes Pop-up statt eines Blatts unter der Konsole: die Sheet-Fläche selbst wird der
         Schleier über die ganze Seite, die Karte darin trägt jetzt das Aussehen, das vorher am
         Blatt hing. Dieselbe Zweiteilung wie beim Anmelden-Pop-up der Einstiegsseite. */
      .prompt-setup-sheet{position:fixed;inset:0;z-index:70;display:flex;align-items:center;justify-content:center;padding:20px;margin:0;background:rgba(8,12,16,.6);backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px)}
      .prompt-setup-sheet[hidden]{display:none!important}
      .prompt-setup-card{width:min(420px,100%);max-height:min(80vh,560px);overflow-y:auto;overscroll-behavior:contain;padding:6px;border:1px solid rgba(255,255,255,.14);border-radius:16px;background:#141e28;box-shadow:0 40px 100px rgba(0,0,0,.5)}
  ...
  function selectMode(mode){const home=ensureHome();if(!home)return;
    ...
    // die Prüfung direkt und lässt die eingestellte Arbeitsart stehen. proxy() würde den echten
    // Kachel-Klick auslösen, den home-entry-ui.js abfängt und dafür noch einmal ein eigenes
    // Beschreibungsfenster aufklappt - hier steht die Beschreibung schon im Startfeld, also
    // direkt über submitBrief('preview', …), das genau diesen Zwischenschritt überspringt.
    if(mode==='check'){closeModeMenu(home);const text=$('#promptCommandInput',home)?.value.trim()||'';if(window.PromptAiHomeEntry?.submitBrief)window.PromptAiHomeEntry.submitBrief('preview',text);else proxy('workspacePreviewBtn');return}
  ...
  function bindHome(home){home.dataset.commandMode='website';$('#promptModeButton',home).addEventListener('click',()=>{const menu=$('#promptModeMenu',home),open=menu.hidden;menu.hidden=!open;$('#promptModeButton',home).setAttribute('aria-expanded',String(open))});home.querySelectorAll('[data-command-mode]').forEach(option=>option.addEventListener('click',()=>selectMode(option.dataset.commandMode)));$('#promptCommandForm',home).addEventListener('submit',submitCommand);
    ...
    // #welcomePage trägt eine transform (und sei es nur die Identität) - das reicht laut
    // Spezifikation, um für fixed-positionierte Nachfahren einen eigenen Bezugsrahmen zu öffnen,
    // statt sie am Viewport ausrichten zu lassen. Das Blatt zieht deshalb direkt unter <body> um,
    // wo es garantiert die ganze Seite abdeckt.
    document.body.appendChild(setupSheet);
    ...
    // Die Sheet-Fläche ist jetzt der Schleier über die ganze Seite - ein Klick darauf (statt auf
    // die Karte darin) zählt wie ein Klick daneben und schließt das Pop-up, genau wie Escape.
    document.addEventListener('click',event=>{
      if(event.target.closest?.('#promptSetupButton'))return;
      if(event.target.closest?.('.prompt-setup-card'))return;
      if(setupSheet.hidden)return;
      setupSheet.hidden=true;setupButton.setAttribute('aria-expanded','false');
    });
    document.addEventListener('keydown',event=>{
      if(event.key!=='Escape'||setupSheet.hidden)return;
      setupSheet.hidden=true;setupButton.setAttribute('aria-expanded','false');
    });
