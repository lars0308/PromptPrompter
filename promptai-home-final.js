(()=>{
  ...
  // Die drei Arbeitsarten stehen offen nebeneinander statt in einer Klappliste: sie sind der
  // eigentliche Anfang jedes Wegs, und aufgeklappt kostet die Wahl einen Klick statt zwei.
  const MODES=[['website','Website erstellen',icons.website],['revision','Website bearbeiten',icons.revision],['free','Freier Prompt',icons.prompt]];
  // Vorschläge sind kein Dekor: sie beantworten die Frage, wie ausführlich ein Auftrag sein darf.
  // Ein Klick füllt das Feld, geschickt wird erst nach dem Nachbessern.
  const STARTERS={
    website:['Landingpage für meinen Handwerksbetrieb','Onlineshop für handgemachte Keramik','Portfolio mit Kontaktformular','Einseiter für ein Restaurant'],
    revision:['Startseite kürzen, Kontakt nach oben','Farben und Schriften moderner','Mobile Ansicht aufräumen','Bilder austauschen und Texte straffen'],
    free:['Prompt für ein Produktfoto','Systemprompt für einen Support-Chat','Prompt für ein kurzes Intro-Video','Prompt für einen Werbetext']
  };
  ...
  function installStyles(){
    ...
    const style=document.createElement('style');style.id='promptAiHomeFinalStyles';style.textContent=`
      ...
      .prompt-command-top{position:relative;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;padding:16px 16px 0}
      .prompt-mode-chip{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;min-height:66px;padding:9px 6px;border:1px solid #33465a;border-radius:13px;background:#141e28;color:#a8bccc;font:700 11.5px/1.25 Arial,Helvetica,sans-serif;text-align:center;cursor:pointer;transition:border-color .16s ease,background .16s ease,color .16s ease}
      .prompt-mode-chip:hover{border-color:#4d637a;background:#18242f;color:#dce9f4}
      .prompt-mode-chip[aria-selected="true"]{border-color:var(--home-blue);background:#1a2a37;color:#eef7fd}
      .prompt-mode-chip[aria-selected="true"] svg{color:var(--home-blue)}
      .prompt-mode-chip svg,.prompt-latest-icon svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}
      .prompt-command-tag{color:#5f7488;font-weight:800;letter-spacing:.16em}
      .prompt-starters{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px}
      .prompt-starter{min-height:38px;padding:0 14px;border:1px solid var(--home-line);border-radius:999px;background:var(--home-card);color:var(--home-muted);font:650 12px/1 Arial,Helvetica,sans-serif;white-space:nowrap;cursor:pointer;transition:border-color .16s ease,color .16s ease}
      .prompt-starter:hover{border-color:var(--home-blue);color:var(--home-ink)}
      .prompt-command-input{display:block;width:100%;min-height:185px;padding:30px 76px 24px 24px;border:0;outline:0;resize:vertical;background:transparent;color:#f2f7fb;font:500 clamp(17px,2.3vw,21px)/1.55 Arial,Helvetica,sans-serif;caret-color:var(--home-blue)}
      .prompt-command-input::placeholder{color:#71808e}
      .prompt-command-submit{position:absolute;right:21px;bottom:69px;display:grid;width:52px;height:52px;min-height:52px;padding:0;place-items:center;border:0;border-radius:13px;background:var(--home-blue);color:#07131c;font-size:25px;cursor:pointer;transition:transform .16s ease,background .16s ease}
  ...
  function modeChips(active){return MODES.map(([mode,label,icon])=>`<button type="button" class="prompt-mode-chip" role="tab" data-command-mode="${mode}" aria-selected="${String(mode===active)}">${icon}<span>${label}</span></button>`).join('')}
  function starterChips(mode){return (STARTERS[mode]||STARTERS.website).map(text=>`<button type="button" class="prompt-starter" data-starter="${text.replace(/"/g,'&quot;')}">${text}</button>`).join('')}
  function markup(){return `<section class="prompt-command-home" aria-label="Prompt.ai Start"><header class="prompt-home-intro"><span class="prompt-system-line">Workspace bereit</span><h1>Hallo <span id="promptHomeName">Lars</span>.</h1><p>Was möchtest du heute umsetzen?</p></header><form class="prompt-command-panel" id="promptCommandForm"><div class="prompt-command-top" id="promptModeRow" role="tablist" aria-label="Arbeitsart wählen">${modeChips('website')}</div><textarea class="prompt-command-input" id="promptCommandInput" rows="5" minlength="8" placeholder="Beschreibe kurz, was entstehen soll …" aria-label="Projekt oder Aufgabe beschreiben"></textarea><button class="prompt-command-submit" id="promptCommandSubmit" type="submit" aria-label="Weiter">→</button><footer class="prompt-command-meta"><span class="prompt-command-tag">COMMAND / 01</span><span>Modell automatisch</span><span><b id="promptHomePlan">Free</b></span><span class="prompt-command-error" id="promptCommandError" role="status"></span></footer></form><div class="prompt-starters" id="promptStarters" aria-label="Vorschläge">${starterChips('website')}</div><section class="prompt-latest" aria-label="Letztes Projekt"><span class="prompt-latest-icon">${icons.folder}</span><div class="prompt-latest-copy"><strong>Letztes Projekt</strong><small id="promptLatestTitle">Gespeicherten Arbeitsstand fortsetzen</small></div><button type="button" class="prompt-latest-action" id="promptLatestAction">Weiterarbeiten →</button></section></section>`}
  ...
  function selectMode(mode,focus=true){const home=ensureHome();if(!home)return;home.dataset.commandMode=mode;const copy=modeCopy(mode),input=$('#promptCommandInput',home);input.placeholder=copy.placeholder;home.querySelectorAll('.prompt-mode-chip').forEach(chip=>chip.setAttribute('aria-selected',String(chip.dataset.commandMode===mode)));const box=$('#promptStarters',home);if(box)box.innerHTML=starterChips(mode);if(focus)input.focus()}
  ...
  function bindHome(home){home.dataset.commandMode='website';
    // Ein Zuhörer am Abschnitt statt einer pro Knopf: die Vorschläge werden bei jedem Moduswechsel
    // neu geschrieben, einzeln gebundene Handler wären danach an gelöschten Knoten gehangen.
    home.addEventListener('click',event=>{
      // Nach der Klasse suchen, nicht nach dem Attribut: der Abschnitt selbst trägt
      // data-command-mode als aktuellen Modus, und closest() wäre bei jedem Klick dort gelandet.
      const chip=event.target.closest?.('.prompt-mode-chip');
      if(chip){selectMode(chip.dataset.commandMode);return}
      const starter=event.target.closest?.('[data-starter]');
      if(!starter)return;
      const input=$('#promptCommandInput',home);
      input.value=starter.dataset.starter;input.focus();
      input.setSelectionRange(input.value.length,input.value.length);
      $('#promptCommandError',home).textContent='';
    });
    $('#promptCommandForm',home).addEventListener('submit',submitCommand);
    $('#promptLatestAction',home).addEventListener('click',()=>proxy('workspaceLastProjectBtn'));
    $('#promptCommandInput',home).addEventListener('input',()=>{$('#promptCommandError',home).textContent=''})}
  function syncHome(){const visible=homeVisible();document.documentElement.classList.toggle('prompt-home-surface',visible);if(!visible)return;const home=ensureHome();if(!home)return;$('#promptHomeName',home).textContent=resolvedName()||'Lars';const access=window.PromptAiAccess||{},plan=access.isAdmin?'Ultimate':String(access.plan||'free');$('#promptHomePlan',home).textContent=plan.charAt(0).toUpperCase()+plan.slice(1);const title=projectTitle(),original=$('#workspaceLastProjectBtn');$('#promptLatestTitle',home).textContent=title||'Gespeicherten Arbeitsstand fortsetzen';$('#promptLatestAction',home).disabled=Boolean(original?.disabled);ensureThemeToggle();const topbar=$('body>.topbar')||$('.topbar');if(topbar){topbar.hidden=false;topbar.removeAttribute('aria-hidden')}}
