-- SiteBrief shares the existing "Stundennachweis app" Supabase project.
-- Preserve access for all users that exist at migration time, while preventing
-- future SiteBrief signups from automatically gaining access to business data.

create table if not exists public.stundennachweis_allowed_users (
  user_id uuid primary key references auth.users(id) on delete cascade,
  created_at timestamptz not null default now()
);

insert into public.stundennachweis_allowed_users (user_id)
select id from auth.users
on conflict (user_id) do nothing;

alter table public.stundennachweis_allowed_users enable row level security;
revoke all on table public.stundennachweis_allowed_users from anon;
grant select on table public.stundennachweis_allowed_users to authenticated;

drop policy if exists "Stundennachweis allowlist membership" on public.stundennachweis_allowed_users;
create policy "Stundennachweis allowlist membership"
on public.stundennachweis_allowed_users
for select to authenticated
using ((select auth.uid()) is not null and (select auth.uid()) = user_id);

-- Replace the former broad "any authenticated user" rules with an allowlist.
-- Existing users were seeded above, so current app access is preserved.
drop policy if exists "Angemeldete Nutzer duerfen Angebote verwalten" on public.angebote;
create policy "Angemeldete Nutzer duerfen Angebote verwalten" on public.angebote
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Auftraege verwalten" on public.auftraege;
create policy "Angemeldete Nutzer duerfen Auftraege verwalten" on public.auftraege
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Kunden verwalten" on public.customers;
create policy "Angemeldete Nutzer duerfen Kunden verwalten" on public.customers
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Mitarbeiter verwalten" on public.employees;
create policy "Angemeldete Nutzer duerfen Mitarbeiter verwalten" on public.employees
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Material verwalten" on public.materials;
create policy "Angemeldete Nutzer duerfen Material verwalten" on public.materials
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Objekte verwalten" on public.objekte;
create policy "Angemeldete Nutzer duerfen Objekte verwalten" on public.objekte
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Protokolle verwalten" on public.protocols;
create policy "Angemeldete Nutzer duerfen Protokolle verwalten" on public.protocols
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Rechnungen verwalten" on public.rechnungen;
create policy "Angemeldete Nutzer duerfen Rechnungen verwalten" on public.rechnungen
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Einstellungen verwalten" on public.settings;
create policy "Angemeldete Nutzer duerfen Einstellungen verwalten" on public.settings
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Stundenzettel verwalten" on public.timesheets;
create policy "Angemeldete Nutzer duerfen Stundenzettel verwalten" on public.timesheets
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));

drop policy if exists "Angemeldete Nutzer duerfen Fahrtenbuch verwalten" on public.trips;
create policy "Angemeldete Nutzer duerfen Fahrtenbuch verwalten" on public.trips
for all to authenticated
using (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())))
with check (exists (select 1 from public.stundennachweis_allowed_users a where a.user_id = (select auth.uid())));
