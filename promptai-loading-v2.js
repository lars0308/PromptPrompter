(()=>{
  ...
  function installStyles(){
    ...
    const s=document.createElement('style');s.id='promptLoadingV2Styles';s.textContent=`
      ...
      .prompt-handoff-loader{position:fixed;z-index:2147483600;inset:0;display:grid;place-items:center;padding:28px 20px;background:var(--paper);color:var(--ink)}
      ...
      .prompt-completion-flash{position:fixed;z-index:2147483646;inset:0;display:grid;place-items:center;padding:28px 20px;background:color-mix(in srgb,var(--paper) 97%,transparent);backdrop-filter:blur(10px);animation:promptFlashIn .14s ease both}.prompt-completion-flash>div{width:min(580px,100%);text-align:center}.prompt-completion-flash .prompt-process-lines{margin-top:22px;text-align:left}.prompt-completion-flash .prompt-process-fill{clip-path:inset(0)!important;color:var(--prompt-load-blue);animation:promptLoadFinish .48s ease both!important}
