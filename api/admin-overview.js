...
module.exports=async function(req,res){
  ...
  try{
    ...
    const [auth,subscriptions,profiles,projects,usage,states,announcements,offers,support,aiConnections,previewAiRoutes,quotaLimits,maintenanceRows,adminRows]=await Promise.all([
    ...
    return res.status(200).json({stats:{users:users.length,activeSubscriptions,projects:projects.length,generations:usage.length},users,usage,announcements,offer:offers[0]||null,support,aiConnections,previewAiRoutes,quotaLimits,maintenance:maintenanceRows[0]||null});
