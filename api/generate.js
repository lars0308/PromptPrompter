...
module.exports=async function generateRouter(req,res){if(req.method==='POST'){const action=String(req.body?.action||'');if(action==='preview-image')return previewImage(req,res);if(action==='sandbox-build')return sandboxBuild(req,res);if(action==='free-prompt')return freePrompt(req,res);if(req.body?.systemAiProfileId&&req.body?.useOwnApi!==true)return runSystemProfiles(req,res)}return core(req,res)};
