...
async function quotaRoute(req,res,action){
  try{
    ...
    if(action==='quota-consume'){
      if(String(req.body?.metric||'')!=='website_generations')return res.status(400).json({error:'Nur Website-Generierungen werden über diesen Ablauf verbraucht.'});
      return res.status(200).json(await consumeWebsiteGeneration(req));
    }
