(()=>{
  ...
  async function sendLearningFeedback(){
    const status=$('#outcomePreviewStatus'),consent=$('#outcomeLearningConsent')?.checked,rating=Number($('#outcomeRating')?.value)||0;if(!lastOutcome){status.className='outcome-preview-status error';status.textContent='Zum Bewerten fehlt noch das gebaute Projekt: lade oben das ZIP oder den Build-Ordner, dann vergleicht Prompt.ai Auftrag und Ergebnis.';return}if(!consent){status.className='outcome-preview-status error';status.textContent='Bitte die freiwillige Lernfreigabe aktivieren oder die Vorschau einfach ohne Auswertung nutzen.';return}if(!rating){status.className='outcome-preview-status error';status.textContent='Bitte das Ergebnis zuerst mit 1 bis 5 bewerten.';return}
