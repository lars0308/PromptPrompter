(()=>{
  ...
  async function test(card){
    ...
    try{const data=await api('/api/admin-action',{method:'POST',body:JSON.stringify({action:'ai-test',provider})}),models=data.models||[];const select=card.querySelector('[data-ai-model]');if(select){const current=select.value;select.innerHTML=(['',...(models.length?models:current?[current]:[])].filter((v,i,arr)=>arr.indexOf(v)===i)).map(v=>`<option value="${esc(v)}">${v?esc(v):'Kein festes Standardmodell'}</option>`).join('');select.value=current}show(`${PROVIDERS[provider].name} funktioniert${models.length?` · ${models.length} Modelle gefunden, im Standardmodell-Feld auswählbar`:''}.`,'good')}catch(error){show(error.message,'error')}
  }
