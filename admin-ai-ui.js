(()=>{
  ...
  function bindCards(){
    $$('#adminAiGrid [data-ai-toggle]').forEach(head=>head.onclick=()=>{
      const card=head.closest('[data-ai-provider]'),id=card.dataset.aiProvider;
      if(card.classList.contains('is-open')&&hasConnection(id))return;
  ...
  function hasConnection(id){return Boolean((state.connections||[]).find(x=>x.provider===id&&x.last4))}
  ...
  async function test(card){
    ...
    try{const data=await api('/api/admin-action',{method:'POST',body:JSON.stringify({action:'ai-test',provider})});show(`${PROVIDERS[provider].name} funktioniert${data.models?.length?` · ${data.models.length} Modelle gefunden`:''}.`,'good')}catch(error){show(error.message,'error')}
  }
