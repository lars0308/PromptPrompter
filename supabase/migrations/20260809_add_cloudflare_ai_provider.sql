alter table public.sitebrief_ai_connections
  drop constraint if exists sitebrief_ai_connections_provider_check;

alter table public.sitebrief_ai_connections
  add constraint sitebrief_ai_connections_provider_check
  check (provider in ('gateway','openai','gemini','cloudflare'));
...
begin
  ...
  if p_provider not in ('gateway','openai','gemini','cloudflare') then raise exception 'Unbekannter KI-Anbieter'; end if;
