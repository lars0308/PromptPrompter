(()=>{
  ...
  function fillFreeFields(brief){
    window.PromptAiFreePrompt?.ensure?.();
    const desc=$('#freePromptDescription');
    if(desc){desc.value=brief;desc.dispatchEvent(new Event('input',{bubbles:true}))}
    const cat=$('#freePromptCategory'),detected=detectCategory(brief);
    if(cat&&[...cat.options].some(o=>o.value===detected)){cat.value=detected;cat.dispatchEvent(new Event('change',{bubbles:true}))}
    return Boolean(desc);
  }
  function openFreeForm(){const b=$('#workspaceFreePromptBtn'),original=freeOriginal||b?.__promptFreeOriginal;if(typeof original==='function')original();else if(b){bypassTarget=b.id;b.click();bypassTarget=''}}
  // Vom Textfeld der Startseite aus ist der freie Prompt ein Satz und ein Klick. Der Fragebogen
  // dahinter fragt Ausgabetyp, Ziel-KI und drei Zusatzfelder ab - beim freien Prompt ist das
  // Beiwerk, das den Weg unterbricht: der Ausgabetyp steht aus der Beschreibung fest, die Ziel-KI
  // ist voreingestellt. Es geht deshalb direkt in die Erstellung, mit Ladeschirm und danach dem
  // fertigen Prompt. Der Fragebogen bleibt fuer den Weg ueber die Kachel, wo man ihn ausdruecklich
  // aufruft - und als Rueckfallweg, falls die Beschreibung fuer einen Direktlauf zu kurz ist.
  async function openFreeWithBrief(brief){
    if(fillFreeFields(brief)&&await window.PromptAiFreePrompt?.runDirect?.())return;
    openFreeForm();setTimeout(()=>fillFreeFields(brief),60);
  }
  async function submitBrief(kind,brief){const text=String(brief||'').trim();if(text.length<8)return false;if(kind==='website'){await window.PromptAiProjectStart?.startFromBrief?.(text);return true}if(kind==='free'){await openFreeWithBrief(text);return true}
