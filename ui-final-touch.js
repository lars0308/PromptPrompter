(()=>{
  ...
  function style(){if($('#promptFinalTouchStyles'))return;const s=document.createElement('style');s.id='promptFinalTouchStyles';s.textContent=`
    .prompt-menu-shield{position:fixed;inset:0;z-index:2147483000;background:rgba(12,17,22,.66);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);opacity:0;pointer-events:none;transition:opacity .2s ease}
  ...
  function syncMenuShield(){const menu=$('#topbarMenu'),shield=$('#promptFinalMenuBackdrop');if(!shield)return;const open=Boolean(menu?.classList.contains('open'));shield.classList.toggle('show',open);shield.setAttribute('aria-hidden',String(!open))}
  ...
  function settle(){style();ensureMenuShield();bind();cleanAuth();cleanFreeStatus()}
