(()=>{
  ...
  // Der freie Prompt hatte zwei eigene Arbeitsanzeigen - eine im Fragebogen, eine im
  // Ergebnisfenster -, waehrend jede andere KI-Wartezeit den grossen Ladeschirm zeigt. Das sah
  // an derselben Stelle unterschiedlich aus, je nachdem, welchen Weg man genommen hatte. Jetzt
  // gilt hier dasselbe wie ueberall: promptai-loading-v2.js legt den Schirm ueber die Anfrage
  // ('free-prompt' steht dort in AI_TASKS), und dieses Fenster kommt erst mit dem Ergebnis.
  let directRun=false;
  ...
  async function runDirect(){
    ...
    const error=$('#freePromptResultError',dialog);if(error){error.hidden=true;error.textContent=''}
    directRun=true;
    try{await generate()}finally{directRun=false}
    return true;
  }
  async function generate(){if(busy)return;const desc=$('#freePromptDescription').value.trim(),status=$('#freePromptStatus');if(desc.length<12){status.className='free-prompt-status error';status.textContent='Beschreibe bitte etwas genauer, was die KI für dich erstellen soll.';return}busy=true;$('#freePromptGenerate').disabled=true;status.className='free-prompt-status free-prompt-loading';status.textContent=paid()?'Prompt.ai analysiert Ziel, Kontext und Ausgabeformat…':'Kompakter Basis-Prompt wird erstellt…';try{const headers=await authHeaders(),r=await fetch('/api/generate',{method:'POST',headers:{'Content-Type':'application/json',...headers},body:JSON.stringify(payload())}),data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Prompt konnte nicht erstellt werden.');const resultDialog=ensureResultDialog();$('#freePromptOutput',resultDialog).value=data.prompt||'';zeigeRohhinweis(resultDialog,data.polished===false);status.className=data.polished===false?'free-prompt-status warn':'free-prompt-status good';status.textContent=data.polished===false?'Keine KI war erreichbar – der Prompt trägt deine Angaben im Originalwortlaut.':data.advanced?`Professioneller Prompt erstellt${data.profile?` · ${data.profile}`:''}.`:'Free-Basis-Prompt erstellt. Mit Pro werden alle Zusatzangaben von der KI analysiert und stärker auf das Ziel-Tool zugeschnitten.';try{resultDialog.showModal()}catch{}window.dispatchEvent(new CustomEvent('promptai:prompt-version',{detail:{source:'freeprompt',title:`Freier Prompt · ${CATEGORIES.find(x=>x[0]===$('#freePromptCategory').value)?.[1]||'Eigener Typ'}`,prompt:data.prompt||''}}))}catch(e){status.className='free-prompt-status error';status.textContent=e.message||'Prompt konnte nicht erstellt werden.';if(directRun){const dialog=ensureResultDialog(),slot=$('#freePromptResultError',dialog);if(slot){slot.textContent=status.textContent;slot.hidden=false}try{if(!dialog.open)dialog.showModal()}catch{}}}finally{busy=false;$('#freePromptGenerate').disabled=false}}
  // Der Server meldet mit `polished:false`, dass keine KI erreichbar war und der Notbehelf
  // geantwortet hat. Dieser Wert wurde nie ausgewertet: darüber stand trotzdem „Professioneller
  // Prompt erstellt“, während im Feld die Eingabe des Kunden lag - nur mit großem Anfangsbuchstaben
  // und einem Punkt am Ende. Wer das kopiert, gibt seine eigenen Worte an die Ziel-KI weiter und
  // hält sie für aufbereitet.
  //
  // Verschweigen ist hier das Schlimmste: der Text ist brauchbar, aber er ist etwas anderes als
  // versprochen. Also steht es dabei, und ein zweiter Versuch ist einen Klick entfernt.
  function zeigeRohhinweis(dialog,roh){
    let hinweis=$('#freePromptRawNote',dialog);
    if(!roh){hinweis?.remove();return}
    if(!hinweis){
      hinweis=document.createElement('p');hinweis.id='freePromptRawNote';hinweis.className='free-prompt-raw-note';
      hinweis.innerHTML='<strong>Nicht von der KI ausformuliert.</strong> Es war gerade keine KI erreichbar. Deine Angaben stehen im Originalwortlaut im Prompt, mit dem Auftrag an die Ziel-KI, sie fachlich zu lesen statt sie abzuschreiben. <button type="button" class="link-btn" id="freePromptRetry">Nochmal versuchen</button>';
      $('#freePromptOutput',dialog)?.insertAdjacentElement('beforebegin',hinweis);
      $('#freePromptRetry',hinweis).onclick=()=>{if(!busy)generate()};
    }
  }
