(()=>{
  ...
  function planQuotaHtml(p){
    const q=LIMITS[p]||LIMITS.free,ai=q.ai_previews?`${q.ai_previews} / Monat`:'Ab Pro';
  ...
  function watchWebsiteResult(beforeStatus,beforeHtml){
    ...
    const timer=setInterval(async()=>{
      ...
      if(changed&&has&&/(erstellt|fertig|lokale Vorschauen|Richtungen)/i.test(text)){clearInterval(timer);if(done)return;done=true;try{cache=await quotaApi('quota-consume',{metric:'website_generations'});cacheAt=Date.now();renderAll()}catch(error){if(error.data?.quota){cache=error.data.quota;cacheAt=Date.now();renderAll()}}return}
  ...
  function patchFetch(){
    ...
    const wrapped=async(input,init)=>{let action='';try{const url=typeof input==='string'?input:input?.url||'';if(String(url).includes('/api/generate')&&init?.body){const body=JSON.parse(init.body);action=String(body?.action||'')}}catch{}const response=await native(input,init);if(['free-prompt','preview-image','quota-consume'].includes(action)){setTimeout(()=>loadSummary(true),250)}return response};wrapped.__quotaWrapped=true;window.fetch=wrapped;
  }
  function observe(){
