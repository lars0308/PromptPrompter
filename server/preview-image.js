...
module.exports=async function previewImage(req,res){res.setHeader('Cache-Control','no-store, private');if(!rateLimit(req,res,{key:'preview-image',limit:12,windowMs:60000}))return;const started=Date.now(),body=req.body||{},project=body.project||{},requested=String(body.imageProvider||'auto').toLowerCase();let usage={action:'preview-image',provider:requested,model:'',project};try{await primePromptTemplates();const ent=await getEntitlements(req);if(ent.plan==='free'&&!ent.isAdmin)return res.status(403).json({error:'KI-Bildvorschauen sind ab Pro verfügbar. HTML-Vorschauen bleiben in Free verfügbar.'});let candidates=await listProfiles('image',{providers:['gateway','openai','gemini','cloudflare'],plan:ent.isAdmin?'ultimate':String(ent.plan||'free')});if(!candidates.length)candidates=fallback();if(body.useOwnApi===true&&['gateway','openai','gemini','cloudflare'].includes(requested))candidates=candidates.filter(x=>x.provider===requested);
